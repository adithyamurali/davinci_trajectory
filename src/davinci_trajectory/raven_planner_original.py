...
from davinci_utils import raven_constants_original as raven_constants
...
class RavenPlanner:
    ...
    def __init__(self, armNames, errorModel=None, thread=True, withWorkspace=False, addNoise=False):
        ...
        ravenFile = os.path.join(roslib.packages.get_pkg_subdir('davinci_trajectory','src/davinci_trajectory'),'myRaven.xml')
        # if withWorkspace:
    ...
    def _init_arm(self, armName):
        ...
        self.raveJointNames[armName] = ['{1}_{0}'.format(armName[0].upper(), name) for name in self.raveJointNamesPrefixes]
